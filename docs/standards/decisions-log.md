...
| Q-C10 | `CcsdsError::SequenceCountOutOfRange` variant | Phase 04 lands `SequenceCountError::SequenceCountOutOfRange(u16)` as a module-local placeholder enforcing the 14-bit `SequenceCount` ceiling per arch §2.2. Arch §2.8's `CcsdsError` enumeration — documented as exhaustive and intentionally not `#[non_exhaustive]` — has no matching variant, so the Phase 05 placeholder fold needs either (a) a tenth `CcsdsError::SequenceCountOutOfRange(u16)` variant with §2.8 amended, or (b) a documented exception that moves the 14-bit check to `PrimaryHeader::decode` and surfaces it through an existing variant. Option (a) is the default unless Phase 05 re-evaluates; flagged from Phase 04 so the fold does not get blocked. | [06-ground-segment-rust.md §2.8](../architecture/06-ground-segment-rust.md) (B3, planned) | pending |
