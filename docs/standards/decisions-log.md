...
| Q-C3 | CFDP Class 2 boundary | Use the `cfdp-core` crate with a transport module. Boundary is the `CfdpProvider` trait in `rust/ground_station/src/cfdp/`. Class 1 implements `CfdpProvider` via `Class1Receiver`; Class 2 lands later as an additional implementation against the same trait. Implementation: `cfdp/mod.rs` (traits + types), `cfdp/class1.rs` (`Class1Receiver`), `cfdp/adapter.rs` (TAI↔Unix epoch). Note: `cfdp-rs 0.3.0` provides no `Daemon` type; `Class1Receiver` is a self-contained state machine using `cfdp-core` as a workspace dep per Phase 26. | [06-ground-segment-rust.md](../architecture/06-ground-segment-rust.md) | resolved |
...
| Q-C8 | Endianness + conversion locus | Big-endian on the wire (CCSDS-aligned). Conversion locus: `cfs_bindings` on the FSW-adjacent Rust side + new crate `ccsds_wire` for pure Rust pack/unpack. No ad-hoc BE↔LE conversion anywhere else. Resolved by Phase 10: `scripts/grep-lints.sh` guards `from_le_bytes\|to_le_bytes` via `rg`; all 7 property tests + 5 KATs confirm BE-only wire encoding. | [06-ground-segment-rust.md](../architecture/06-ground-segment-rust.md) | resolved |
...
| Q-F3 | EDAC / memory protection hooks | C-side anchor = `__attribute__((section(".critical_mem")))`. Rust-side anchor = dedicated `Vault<T>` wrapper. EDAC is primarily an FSW C concern; `Vault<T>` provides a parallel abstraction on the ground/relay Rust side for future cross-validation. **Phase 28 anchor:** `Cop1Engine::vs` (V(S) uplink sequence counter in `uplink/cop1.rs`) is the FOP-1 sequence anchor — analogous to the CFDP TX counter. Marked with Q-F3 comment as a reserved `Vault<u8>` site when `rust/vault/` lands (Phase C+). | [09-failure-and-radiation.md §5](../architecture/09-failure-and-radiation.md) | resolved |
