...
export function TitanCommArrows() {
  const vehicles = useTitanStore((s) => s.vehicles)
  const activeComms = useTitanStore((s) => s.activeComms)
  ...
  return (
    <>
      {activeComms.map((evt) => {
        ...
        const vehicleType = vehicles.find((v) => v.id === evt.fromId)?.type ?? 'uav'
        const color = TITAN_VEHICLE_COLOR[vehicleType]
        return (
          <Arrow
            key={evt.id}
            from={from}
