...
export function MarsGroundVehicle({ vehicle }: Props) {
  ...
  const liveVehicle = useMarsStore((s) => s.vehicles.find((v) => v.id === vehicle.id) ?? vehicle)
    ...
    >
      ...
      {isSelected && (
        <>
          ...
          <MarsVehicleInfoBox vehicle={liveVehicle} />
        </>
      )}
    </group>
  )
