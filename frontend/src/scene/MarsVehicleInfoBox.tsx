...
const ICONS: Record<string, string> = { drone: '🛸', land: '🚙', cryobot: '🔩' }
const ROLES: Record<string, string> = { drone: 'AERIAL DRONE', land: 'SURFACE ROVER', cryobot: 'ICE DRILLER' }
...
export function MarsVehicleInfoBox({ vehicle }: { vehicle: MarsVehicle }) {
  const selectVehicle = useMarsStore((s) => s.selectVehicle)
  const color = VEHICLE_COLOR[vehicle.type]
    ...
    <Html position={[0, 2.6, 0]} center distanceFactor={12} zIndexRange={[1000, 0]}>
      ...
      >
        ...
        <div style={{ marginTop: '10px', paddingTop: '7px', borderTop: `1px solid ${color}20`, display: 'flex', alignItems: 'center', gap: '7px' }}>
          ...
          <span style={{ color: '#374151', fontSize: '10px', marginLeft: 'auto' }}>MARS SOL</span>
        </div>
      </div>
    </Html>
  )
