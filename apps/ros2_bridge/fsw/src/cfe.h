...
/*
 * cfe.h — Minimal cFE / OSAL type and constant stubs for ros2_bridge
 * standalone builds.
 *
 * Extends the sim_adapter stub pattern with:
 *   - OS_SocketAddrFromString (address string → OSAL addr)
 *   - OS_SocketSendTo         (OSAL datagram send)
 *   - OS_TaskDelay            (OSAL sleep in ms)
 *   - CFE_SB_POLL             (non-blocking SB receive constant)
 *
...
#define CFE_SB_POLL          ((int32) 0)   /* non-blocking receive */
