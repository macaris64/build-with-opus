...
fn main() {
    ...
    // Re-run if either mission header changes.
    println!("cargo:rerun-if-changed=../../_defs/mission_config.h");
    println!("cargo:rerun-if-changed=../../_defs/mids.h");

    let mission_bindings = bindgen::Builder::default()
    ...
    mission_bindings
        ...
        .expect("failed to write mission_config_bindings.rs");

    let mids_bindings = bindgen::Builder::default()
        .header("../../_defs/mids.h")
        .parse_callbacks(Box::new(bindgen::CargoCallbacks::new()))
        .generate()
        .expect("bindgen failed to generate bindings from mids.h");
    mids_bindings
        .write_to_file(out_path.join("mids_bindings.rs"))
        .expect("failed to write mids_bindings.rs");
}
